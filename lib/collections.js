...
var createThumb = function(fileObj, readStream, writeStream) {
    gm(readStream, fileObj.name()).resize('400', '260').stream().pipe(writeStream);
};

Images = new FS.Collection("images", {
  stores: [new FS.Store.FileSystem("images", {transformWrite: createThumb,path: "/tmp/images"})],
...
ProfileImages = new FS.Collection("profileImages", {
  ...
  stores: [new FS.Store.FileSystem("profileImages", {path: "/tmp/images/profile"})],
  filter: {
    allow: {
      maxSize: 2500000,
      contentTypes: ['image/*']
...
Images.allow({
  'insert': function (userId,project) {
    ...
    // if (Roles.userIsInRole(userId, ["admin","default"])) {
    return true;
